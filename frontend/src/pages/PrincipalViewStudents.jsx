...
const PrincipalViewStudents = () => {
    ...
    const fetchStudentDetails = async () => {
        setLoading(true);
        try {
            const response = await axios.get('https://niepid-yrdn.onrender.com/principle/viewstudents', {
                headers: {
    ...
    const fetchTeacherDetails = async (classId) => {
        try {
            console.log("Hello")
            const response = await axios.get(`https://niepid-yrdn.onrender.com/principle/teacher/${classId}`, {
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${localStorage.getItem("token")}`
                },
